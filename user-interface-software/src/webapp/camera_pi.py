...
try:
    from config import (
        CAMERA_THREAD_WIDTH, CAMERA_THREAD_HEIGHT, CAMERA_THREAD_FPS,
        CAMERA_INIT_TIMEOUT_S, CAMERA_FRAME_WAIT_SLEEP_S,
        STROBE_DEFAULT_PERIOD_NS, STROBE_MAX_PERIOD_NS,
        STROBE_PRE_PADDING_NS, STROBE_POST_PADDING_NS,
        STROBE_REPLY_PAUSE_S,
        CAMERA_TYPE_NONE, CAMERA_TYPE_RPI,
        SNAPSHOT_FOLDER, SNAPSHOT_FILENAME_PREFIX, SNAPSHOT_FILENAME_SUFFIX,
        FPS_OPTIMIZATION_MAX_TRIES, FPS_OPTIMIZATION_CONVERGENCE_THRESHOLD_US,
        FPS_OPTIMIZATION_POST_PADDING_OFFSET_US,
        WS_EVENT_CAM, WS_EVENT_STROBE, WS_EVENT_ROI,
        CMD_SNAPSHOT, CMD_OPTIMIZE, CMD_HOLD, CMD_ENABLE, CMD_TIMING,
        CMD_SET, CMD_GET, CMD_CLEAR
    )
except ImportError:
...
class Camera:
    ...
    def __init__(self, exit_event: threading.Event, socketio: Any) -> None:
        ...
        self.strobe_cam = PiStrobeCam(picommon.PORT_STROBE, STROBE_REPLY_PAUSE_S)
    ...
    def initialize(self) -> None:
        ...
        if not hasattr(self.camera, 'cam_running_event') or not self.camera.cam_running_event.is_set():
            try:
                self.camera.start()
            except Exception as e:
    ...
    def optimize_fps(self) -> None:
        ...
        while abs(cam_read_time_us - cam_read_time_us_prev) > FPS_OPTIMIZATION_CONVERGENCE_THRESHOLD_US:
            ...
            try:
                ...
                strobe_post_padding_ns = (cam_read_time_us + FPS_OPTIMIZATION_POST_PADDING_OFFSET_US) * 1000
                logger.debug(f"FPS optimization: read_time={cam_read_time_us}us, padding={strobe_post_padding_ns}ns")
            except Exception as e:
    ...
    def update(self) -> None:
        ...
        try:
            self.strobe_framerate = int(self.strobe_cam.camera.config.get("FrameRate", CAMERA_THREAD_FPS))
    ...
    def on_roi(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if cmd == CMD_SET:
                ...
                logger.info(f"ROI set: ({self.roi['x']}, {self.roi['y']}) "
                ...
                # Send current ROI to requesting client
                if self.socketio:
                    if self.roi:
                        self.socketio.emit(WS_EVENT_ROI, {'roi': self.roi})
                    else:
                ...
                # Only log if ROI was actually set (avoid spam)
