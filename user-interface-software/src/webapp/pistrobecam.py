...
import picommon  # Use picommon.GPIO which handles simulation mode

# Add droplet_detection to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
from droplet_detection import create_camera, BaseCamera

# Use GPIO from picommon (handles simulation mode automatically)
GPIO = picommon.GPIO
...
class PiStrobeCam:
    ...
    def __init__(self, port: int, reply_pause_s: float = STROBE_REPLY_PAUSE_S,
        ...
        # Initialize camera using abstraction layer (auto-detects 32-bit vs 64-bit)
        try:
