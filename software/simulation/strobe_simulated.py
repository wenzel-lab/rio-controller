...
class SimulatedStrobe:
    ...
    # Packet types (matching real firmware - see drivers/strobe.py)
    ...
    def __init__(self, device_port: int, reply_pause_s: float = DEFAULT_REPLY_PAUSE_S):
        ...
        logger.debug(
            f"SimulatedStrobe initialized (port={device_port}, reply_pause={reply_pause_s}s)"
        )
    ...
    def packet_write(self, type_: int, data: list) -> None:
        ...
        try:
            handlers = {
                ...
                self.PACKET_TYPE_SET_HOLD: self._handle_set_hold,
                self.PACKET_TYPE_SET_TRIGGER_MODE: self._handle_set_trigger_mode,
            }
    ...
    def _handle_set_enable(self, data: list) -> None:
        """Handle SET_ENABLE packet."""
        if len(data) >= 1:
            self.enabled = bool(data[0])
            logger.debug(f"Strobe enable set to: {self.enabled}")

    def _handle_set_timing(self, data: list) -> None:
        """Handle SET_TIMING packet."""
        if len(data) >= 8:
    ...
    def _handle_set_hold(self, data: list) -> None:
        """Handle SET_HOLD packet."""
        if len(data) >= 1:
            self.hold = bool(data[0])
            logger.debug(f"Strobe hold set to: {self.hold}")

    def _handle_set_trigger_mode(self, data: list) -> None:
        """Handle SET_TRIGGER_MODE packet."""
        if len(data) >= 1:
    ...
    def get_enable(self) -> Tuple[bool, bool]:
        """Get enable state."""
        # Note: Real firmware uses PACKET_TYPE_SET_ENABLE (1) for both set and get
        valid, data = self.packet_query(self.PACKET_TYPE_SET_ENABLE, [])
    ...
    def set_timing(self, wait_ns: int, period_ns: int) -> Tuple[bool, int, int]:
        ...
        try:
            ...
            logger.warning("Failed to set strobe timing: invalid response")
            return False, 0, 0
        except (ValueError, IndexError, TypeError) as e:
    ...
    def get_timing(self) -> Tuple[bool, int, int]:
        """Get strobe timing."""
        # Note: Real firmware uses PACKET_TYPE_SET_TIMING (2) for both set and get
        valid, data = self.packet_query(self.PACKET_TYPE_SET_TIMING, [])
        if valid and len(data) >= 9:
            # Response format: [0] + wait_ns (4 bytes) + period_ns (4 bytes)
            wait_ns = int.from_bytes(data[1:5], "little", signed=False)
            period_ns = int.from_bytes(data[5:9], "little", signed=False)
    ...
    def get_hold(self) -> Tuple[bool, bool]:
        """Get hold state."""
        # Note: Real firmware uses PACKET_TYPE_SET_HOLD (3) for both set and get
        valid, data = self.packet_query(self.PACKET_TYPE_SET_HOLD, [])
        if valid and len(data) >= 1:
            return True, bool(data[0])
        return False, False
    ...
    def set_trigger_mode(self, hardware_trigger: bool) -> bool:
        ...
        try:
            valid, data = self.packet_query(
                self.PACKET_TYPE_SET_TRIGGER_MODE, [1 if hardware_trigger else 0]
            )
            success = valid and len(data) > 0 and (data[0] == 0)
            if not success:
