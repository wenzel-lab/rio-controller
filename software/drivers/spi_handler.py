...
if SIMULATION_MODE:
    ...
    try:
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"Hardware libraries not available ({e}). Enable simulation mode with RIO_SIMULATION=true")
        raise
...
def spi_init( bus, mode, speed_hz ):
  ...
  if SIMULATION_MODE:
    ...
    _simulated_spi_handler = SimulatedSPIHandler(bus=bus, mode=mode, speed_hz=speed_hz, pin_mode=GPIO.BOARD)
    spi = _simulated_spi_handler.spi  # Access the simulated SPI device - this assigns to global spi
