...
class PiStrobeCam:
            ...
            if self.camera is not None:
                ...
                try:
                    ...
                    if "not initialized" in str(e).lower():
                        logger.warning(f"Camera hardware not available: {e}. Continuing without camera.")
                        self.camera = None
    ...
    def set_timing(self, pre_padding_ns: int, strobe_period_ns: int, post_padding_ns: int) -> bool:
        ...
        try:
            ...
            # Ensure camera is started (but don't force strobe enable)
    ...
    def _calculate_camera_timing(
        ...
        if framerate > MAX_FRAMERATE:
            ...
            logger.debug(f"Framerate clamped to {MAX_FRAMERATE} FPS, shutter adjusted to {shutter_speed_us}us")
    ...
    def _get_actual_shutter_speed(self, expected_shutter_speed_us: int) -> int:
        ...
        if self.camera and hasattr(self.camera, "get_actual_shutter_speed"):
            try:
                ...
                # Fallback to config value if readback method not available
                try:
                    config_shutter = self.camera.config.get("ShutterSpeed", expected_shutter_speed_us)
